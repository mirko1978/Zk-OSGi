...
public class BridgeServlet extends HttpServlet {
	...
	private static FrameworkLauncher framework;
	...
	public void init() throws ServletException {
		...
		String enableFrameworkControlsParameter = getServletConfig()
				.getInitParameter("enableFrameworkControls"); //$NON-NLS-1$
		enableFrameworkControls = (enableFrameworkControlsParameter != null && enableFrameworkControlsParameter
				.equals("true")); //$NON-NLS-1$

		String frameworkLauncherClassParameter = getServletConfig()
				.getInitParameter("frameworkLauncherClass"); //$NON-NLS-1$
		if (frameworkLauncherClassParameter != null) {
			try {
				...
				framework = (FrameworkLauncher) frameworkLauncherClass
						.newInstance();
			} catch (Exception e) {
	...
	/**
	 * destroy() is called by the Servlet Container and used to first stop and
	 * then destroy the framework.
	 */
	public void destroy() {
		framework.stop();
		framework.destroy();
	...
	protected void service(HttpServletRequest req, HttpServletResponse resp)
		...
		if (req.getAttribute(INCLUDE_REQUEST_URI_ATTRIBUTE) == null) {
			...
			String includePathInfo = (String) req
					.getAttribute(INCLUDE_PATH_INFO_ATTRIBUTE);
			// Check if this is being handled by an extension mapping
		...
		if (servletReference == null) {
			resp.sendError(HttpServletResponse.SC_NOT_FOUND,
					"BridgeServlet: " + req.getRequestURI()); //$NON-NLS-1$
			return;
		}
		try {
			Thread.currentThread().setContextClassLoader(
					framework.getFrameworkContextClassLoader());
	 ...
	 * (identified by the request's pathinfo) sp_deploy - Copies the contents of
	 * the Equinox application to the install area sp_undeploy - Removes the
	 * copy of the Equinox application from the install area sp_redeploy -
	 * Resets the platform (e.g. stops, undeploys, deploys, starts) sp_start -
	 * Starts a deployed platform sp_stop - Stops the platform
	 */
	private boolean serviceFrameworkControls(HttpServletRequest req,
		...
		if (pathInfo.equals("/sp_start")) { //$NON-NLS-1$
			...
			if (delegate == null)
				...
				resp.getWriter()
						.write("Servlet delegate registered - " + delegate.getClass().getName()); //$NON-NLS-1$
			return true;
		}
		return false;
	}
	 ...
	 * init(ServletConfig) will be called on the servletDelegate before
	 * returning.
	 * 
	 * @param servletDelegate
	 *            - the servlet to register for delegation
	 */
	public static synchronized void registerServletDelegate(
		...
		if (servletDelegate == null)
			throw new NullPointerException(
					"cannot register a null servlet delegate"); //$NON-NLS-1$

		synchronized (instance) {
			if (instance.delegate != null)
				throw new IllegalStateException(
						"A Servlet Proxy is already registered"); //$NON-NLS-1$

			try {
				servletDelegate.init(instance.getServletConfig());
			} catch (ServletException e) {
				instance.getServletContext().log(
						"Error initializing servlet delegate", e); //$NON-NLS-1$
				return;
			}
	 ...
	 * runtime to unregister a delegate. If the servletDelegate matches the
	 * current registered delegate destroy() is called on the servletDelegate.
	 * destroy() will not be called until the delegate is finished servicing any
	 * previous requests.
	 * 
	 * @param servletDelegate
	...
	public static synchronized void unregisterServletDelegate(
		...
		synchronized (instance) {
			if (instance.delegate == null)
				throw new IllegalStateException(
						"No servlet delegate is registered"); //$NON-NLS-1$
	...
	static class IncludedExtensionMappingRequest extends
	...
	/**
	 * Added for managing the Zk HttpSessionListener
	 * 
