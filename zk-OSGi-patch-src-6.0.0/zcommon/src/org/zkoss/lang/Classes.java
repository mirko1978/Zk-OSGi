...
import org.zkoss.common.zcommon.Activator;
...
public class Classes {
	...
	/**
	 * Instantiates a new instance of the specified class with the specified
	 ...
	 * @param argTypes
	 *            the argument types of the constructor to inovke
	 * @param args
	 ...
	 */
	public static final Object newInstance(Class<?> cls, Class<?>[] argTypes,
	 ...
	 * 
	 * <p>
	 * It searches all contructor for the first one that matches the specified
	 * arguments.
	 * 
	 * @since 3.0.1
	 */
	public static final Object newInstance(Class<?> cls, Object[] args)
		...
		throw new NoSuchMethodException(cls.getName() + ": no constructor for "
				+ Objects.toString(args));
	}

	/**
	 * @param loosely
	 *            whether to match Long with Integer and so on.
	 */
	private static Constructor match(Constructor[] cxs, Object[] args,
			boolean loosely) {
	 ...
	 *            whether to match Long with Integer and so on.
	 */
	private static Method match(Class<?> cls, String name, Object[] args,
			boolean loosely) {
		...
		for (int j = 0; j < ms.length; ++j) {
			...
			if (Modifier.isPublic(ms[j].getDeclaringClass().getModifiers()))
				return ms[j]; // found
			try {
				return getMethodInPublic(cls, ms[j].getName(),
						ms[j].getParameterTypes());
		...
		if (types.length == args.length) {
			...
			for (int k = args.length;;) {
				...
				if (type.isInstance(arg)
						|| (type.isPrimitive() && Primitives.toWrapper(type)
								.isInstance(arg)))
					continue; // matched
	...
	/**
	 * Instantiates a new instance of the specified class name with the
	 * specified arguments.
	 * 
	 * <p>
	 ...
	 */
	public static final Object newInstance(String clsName, Class<?>[] argTypes,
			Object[] args) throws NoSuchMethodException,
			InstantiationException, InvocationTargetException,
	 ...
	 * the specified arguments, by use of {@link #forNameByThread}.
	 * 
	 * <p>
	 * It uses {@link #forNameByThread} to get the class.
	 * 
	 * @param clsName
	 *            the class name of the instance to create
	 * @param argTypes
	...
	/**
	 * Creates and initializes a new instance of the specified class name with
	 * default constructor, by use of {@link #forNameByThread}.
	 */
	public static final Object newInstanceByThread(String clsName)
			throws NoSuchMethodException, InstantiationException,
	...
	/**
	 * Returns the Class object of the specified class name, using the current
	 * thread's context class loader.
	 * <p>
	 * It first tries Thread.currentThread().getContextClassLoader(), and then
	 * {@link Classes}'s class loader if not found.
	 * 
	 * <p>
	...
	public static final Class<?> forNameByThread(String clsName)
		...
		return Activator.forName(clsName);
	}
	...
	public static final String toInternalForm(String clsName) {
		...
		if (elm.length() == 0)
			throw new IllegalArgumentException("Not a legal class name: \""
					+ clsName + '"');
			...
			if (ch == '[') {
				if (leftb)
					throw new IllegalArgumentException(
							"Not a legal class name: \"" + clsName + '"');
				leftb = true;
				sb.append('[');
			} else if (ch == ']') {
				if (!leftb)
	 ...
	 * implements or extends the specified interface. For example, if A extends
	 * B, and C implements A, then getTopInterface(C, B) returns A.
	 * 
	 * <p>
	 * The interfaces implemented by the specified class is checked first, and
	 * then the subclass.
	 * 
	 * @param cls
	 ...
	 * @param subIF
	 *            the sub-interface
	 * @return the topmost interface extending subIF, or null if subIF is not
	 *         implemented by cls
	 */
	 ...
	 * <p>
	 * Unlike {@link Class#getInterfaces}, it recursively searches for all
	 * derived classes.
	 */
	...
	/**
	 * Tests whether a class contains the specified method. Only public methods
	 * are tested.
	 * 
	 * @param cls
	 *            the class to test
	 ...
	 */
	public static final boolean containsMethod(Class<?> cls, String name,
			Class<?>[] paramTypes) {
		try {
	...
	/**
	 * Corrects a string to a valid Java name. Currently, it only removes '-'
	 * and capitalizes the succeeding character. Example, 'field-name' becomes
	 * 'fieldName'.
	 */
	 ...
	 * 
	 * <p>
	 * toMethodName("true", "is") => "isTrue"<br>
	...
	/**
	 * Tests if a method name is an attribute, i.e., prefixing with is, get or
	 * set. Caller could then test if it is a setter or getter by
	 * charAt(0)=='s'.
	 * 
	 * <p>
	...
	public static final boolean isAttribute(String methodName) {
		...
		case 's':
		case 'g':
			if (len < 3 || methodName.charAt(1) != 'e'
	...
	/**
	 * Converts a method name to an attribute name by removing the prefix is,
	 * get or set, or null if it doesn't start with is, get or set.
	 * 
	 * <p>
	 ...
	 * This info describe the method return type, method name and two
	 * collections for arguments type and arguments name;
	 */
	public static class MethodInfo {
		...
		// constructor
		protected MethodInfo(String r, String m, String[] argTs,
				String[] argNs, String tEx) {
			returnType = r;
			method = m;
			argTypes = argTs;
			argNames = argNs;
	...
	/**
	 * Gets the method information from a signature. It returns a method info
	 * with the return type, method name and two collections of arguments type
	 * and arguments name.
	 * 
	 * @param signature
	 *            the method signature.
	 * @return MethodInfo The method information including return type, method
	 *         name and two collections for argument type annd arguments name.
	 */
	public static final MethodInfo parseMethod(String signature)
		...
		return new MethodInfo(returnType, method,
				...
				argNames.toArray(new String[argNames.size()]), tEx);
	}

	/**
	 * Gets the method based on the signature with a class resolver. It also
	 * returns the parameter names to the params list.
	 * 
	 * <p>
	 * Like {@link #getMethodInPublic(Class, String, Class[])}, it returns only
	 * public method in a public class/interface.
	 * 
	 * <p>
	 * For example, "find(java.lang.String name)" will return the method with
	 * one String-typed argument and params will hold "name". The return type is
	 * optional (actualy ignored).
	 * 
	 * <p>
	 * If params is null, the parameter names are not returned and the signature
	 * could be simplified as "find(java.lang.String)".
	 * 
	 * <p>
	 * A cache mechanism is implemented, so you don't need to cache it again in
	 * the caller.
	 * 
	 * @param cls
	 *            the class to look
	 * @param signature
	 *            the method signature; the return type is optional.<br/>
	 *            Notice that the argument's type must be a full-qualified class
	 *            name, unless its package is java.lang or it can be resolved by
	 *            <code>resolver</code>.
	 * @param params
	 *            the collection to hold the parameter names returned; null
	 *            means no parameter names to return
	 * @param resolver
	 *            the class resolver used to resolve the class specified in the
	 *            signature. Ignored if null.
	 * @since 6.0.0
	 */
	public static final Method getMethodBySignature(Class<?> cls,
			String signature, Collection<String> params, ClassResolver resolver)
	 ...
	 *            the method signature; the return type is optional<br/>
	 *            Notice that the argument's type must be a full-qualified class
	 *            name, unless its package is java.lang.
	 * @param params
	 *            the collection to hold the parameter names returned; null
	 *            means no parameter names to return
	 */
	public static final Method getMethodBySignature(Class<?> cls,
			String signature, Collection<String> params)
			throws NoSuchMethodException, ClassNotFoundException {
		return getMethodBySignature(cls, signature, params, null);
	}

	/**
	 * Resolves a class for the method signature. This method will try
	 * java.lang.X if X is not found.
	 */
	private static final Class getClassOfSignature(ClassResolver resolver,
			String clsnm) throws ClassNotFoundException {
		try {
			return resolver != null ? resolver.resolveClass(clsnm)
					: forNameByThread(clsnm);
	 ...
	 * 
	 * <p>
	 * Class.getMethod returns a public method but the class itself might not be
	 * public. However, in many cases, that class also implements a public
	 * interface or class.
	 * 
	 * <p>
	 * This method will search all its public classes to look for the method
	 * that is 'real' public.
	 * 
	 * <p>
	 * NoSuchMethodException is thrown if no public class/interface is found to
	 * have the method.
	 */
	public static final Method getMethodInPublic(Class<?> cls, String name,
	...
	private static NoSuchMethodException newNoSuchMethodException(Class cls,
		...
		return new NoSuchMethodException(cls.getName() + ": no method called "
				+ name + " for " + Objects.toString(args));
	}

	/**
	 * Gets one of the close method by specifying the arguments, rather than the
	 * argument types. It actually calls {@link #getCloseMethod}.
	 */
	public static final Method getMethodByObject(Class<?> cls, String name,
			Object[] args) throws NoSuchMethodException {
		if (args == null)
	...
	/**
	 * Gets one of the close methods -- a close method is a method with the same
	 * name and the compatible argument type. By compatiable we mean the real
	 * method's argument type is the same as or a superclass of the specified
	 * one.
	 * 
	 * <p>
	 * It might not be the best fit one, unless there is a method whose argument
	 * types are exactly argTypes.
	 * 
	 * <p>
	 * You might specify the exact number in argTypes. If any of them is
	 * unknwon, use null. Example, in the following, the first argument could be
	 * anything and the second is anything deriving from MyClass:<br>
	 * <code>new Class[] {null, MyClass.class}</code>
	 * 
	 * <p>
	 * Note: if an argument accepts int, then Integer is considered as
	 * compatible (unlike Class.getMethod). So are long, byte...
	 * 
	 * <p>
	 * A cache mechanism is implemented, so you don't need to cache it again in
	 * the caller.
	 * 
	 * @param cls
	 *            the class to locate the method
	 * @param name
	 *            the method name
	 * @param argTypes
	 *            an array of the argument classes; null to denote no argument
	 *            at all (i.e., exact match). Any argTypes[i] could be null to
	 *            denote any class.
	 * @return the method
	 * @exception NoSuchMethodException
	 *                if the method is not found
	 */
	public static final Method getCloseMethod(Class<?> cls, String name,
			Class<?>[] argTypes) throws NoSuchMethodException {
	...
	/**
	 * Like {@link #getCloseMethod} to get a 'close' method, but it look for
	 * subclass of the arguement (instead of superclass). In other words, it
	 * looks for the method whose argument type is the same as or a subclass of
	 * the specified one.
	 */
	public static final Method getCloseMethodBySubclass(Class<?> cls,
			String name, Class<?>[] argTypes) throws NoSuchMethodException {
		if (argTypes == null || argTypes.length == 0)
			return getMethodInPublic(cls, name, null);
	...
	private static Cache<AOInfo, Object> _closms = new FastReadCache<AOInfo, Object>(
			Library.getIntProperty(
					"org.zkoss.lang.Classes.methods.cache.maxSize", 600),
			4 * 60 * 60 * 1000);

	private static final Method myGetCloseMethod(final Class<?> cls,
			final String name, final Class<?>[] argTypes,
			final boolean bySubclass) throws NoSuchMethodException {
		...
		for (int j = 0; j < ms.length; ++j) {
			...
			final boolean bPublic = Modifier.isPublic(ms[j].getDeclaringClass()
					.getModifiers());
			for (int k = 0;; ++k) {
	...
	/**
	 * Returns all close methods that match the specified condition, or a
	 * zero-length array if none is found.
	 * <p>
	 * Unlike {@link #getCloseMethod}, we don't cache the searched result, and
	 * it won't throw any exception.
	 */
	public static final Method[] getCloseMethods(Class<?> cls, String name,
			Class<?>[] argTypes) {
		if (argTypes == null || argTypes.length == 0) {
			try {
	...
	/**
	 * Like {@link #getCloseMethods} to get all 'close' methods, but it look for
	 * subclass of the arguement (instead of superclass). In other words, it
	 * looks for the method whose argument type is the same as or a subclass of
	 * the specified one.
	 */
	public static final Method[] getCloseMethodsBySubclass(Class<?> cls,
	...
	private static final Method[] myGetCloseMethods(final Class<?> cls,
			final String name, final Class<?>[] argTypes,
			final boolean bySubclass) {
		// assert argTypes != null: "Caller shall handle null";
			...
			for (int k = 0;; ++k) {
				if (k == argTypes.length) { // all matched
					if (bPublic) {
						...
						try {
							mtds.add(getMethodInPublic(cls, ms[j].getName(),
									ms[j].getParameterTypes()));
	 ...
	 * 
	 * <p>
	 * The search sequence is: (assume field name is body)><br>
	 * getBody(...)<br>
	 * isBody(...)<br>
	 * body(...)<br>
	 * body
	 * 
	 * <p>
	 * If B_SET is specified, setBody(...) is searched instead of getBody(...)
	 * and isBody(...). The field is searched only if argsType.length is 0 or 1.
	 * 
	 * <p>
	 * Note: it uses {@link #getCloseMethod} to get the method.
	 * 
	 * <p>
	 * A cache mechanism is implemented, so you don't need to cache it again in
	 * the caller.
	 ...
	 * @exception NoSuchMethodException
	 *                if neither the set method of specified field nor the field
	 *                itself not found
	 * @exception SecurityException
	 *                if access to the information is denied
	 */
	public static final AccessibleObject getAccessibleObject(Class<?> cls,
			String name, Class<?>[] argTypes, int flags)
	...
	private static final AccessibleObject myGetAcsObj(Class<?> cls,
			String name, Class<?>[] argTypes, int flags)
			throws NoSuchMethodException {
		// try public set/get
		final String decoratedName = toMethodName(name,
				(flags & B_SET) != 0 ? "set" : "get");
		try {
		...
		try {
			...
			if ((flags & (B_PUBLIC_ONLY | B_METHOD_ONLY)) == (B_PUBLIC_ONLY | B_METHOD_ONLY))
				throw ex;
		}
	...
	/**
	 * Gets the specified method by searching all methods including <i>any</i>
	 * access control and any base class. Note: you rarely need to call this
	 * metod. In most cases, Class.getMethod, {@link #getCloseMethod}, and
	 * {@link #getMethodInPublic} are what you need.
	 * 
	 * <p>
	 * The search sequence is: this class's methods, and then the superclass's
	 * methods.
	 * 
	 * <p>
	 * Note: public methods don't be treated different. If the caller wants to
	 * search public methods first, it has to call Class.getMethod first.
	 * 
	 * @param cls
	 *            the class to search
	 * @param name
	 ...
	 */
	public static final Method getAnyMethod(Class<?> cls, String name,
			Class<?>[] argTypes) throws NoSuchMethodException {
		try {
	...
	/**
	 * Gets the specified field by searching all fields including any access
	 * control and any base class. The search sequence is: this class's fields,
	 * and then the superclass's fields.
	 * 
	 * <p>
	 * Note: public fields don't be treated different. If the caller wants to
	 * search public fields first, it has to call Class.getField first.
	 * 
	 * @param cls
	 *            the class to search
	 * @param name
	...
	/**
	 * Searches thru each element of the specified array of classes, and returns
	 * classes that are super-classes (or equal) of the sepcified class.
	 * 
	 * @param cls
	 *            the specified class; null is not allowed
	 * @param clsToCheck
	 *            the class array to check; null is acceptable
	 * @return a subset of clsToCheck that are super-class of cls; null if no
	 *         one qualified
	 */
	public static final Class<?>[] getSuperClasses(Class<?> cls,
			Class<?>[] clsToCheck) {
		if (clsToCheck != null) {
	...
	public static final boolean isPrimitiveWrapper(Class<?> cls) {
		return Objects.equals(cls.getPackage(), Boolean.class.getPackage())
				&& (cls.equals(Boolean.class) || cls.equals(Byte.class)
						...
						|| cls.equals(Integer.class) || cls.equals(Long.class) || cls
							.equals(Short.class));
	}
	 ...
	 * 
	 * @param extend
	 *            whether to consider Date, char, boolean, Character and Boolean
	 *            as a numeric object.
	 */
	public static final boolean isNumeric(Class<?> cls, boolean extend) {
		...
		return extend
				&& (cls.equals(Date.class) || cls.equals(Boolean.class) || cls
						.equals(Character.class));
	}

	/**
	 * Converts an object to the specified class. It is the same as coerce(cls,
	 * val, true).
	 * 
	 * @param val
	 *            the value.
	...
	public static Object coerce(Class<?> cls, Object val)
		...
		if (String.class == cls) {
			...
			if (val == null) {
				...
				return s.length() > 0 ? new Character(s.charAt(0))
						: Objects.NULL_CHARACTER;
			}
			...
			if (val == null) {
				...
				try {
					...
					final ClassCastException t = new ClassCastException(
		...
		throw new ClassCastException(Messages.get(
				MCommon.CLASS_NOT_COMPATIABLE, new Object[] {
						val != null ? val + "(" + val.getClass().getName()
								+ ")" : "null", cls }));
	}
	 ...
	 * 
	 * @param nullable
	 *            whether the result could be null. If false, 0 is used for
	 *            number, the default constructor is used for others.
	 *            {@link #coerce(Class, Object)} is a special case that is
	 *            equivalent to nullable=true.
	 * @exception ClassCastException
	 *                if failed to convert
	 */
