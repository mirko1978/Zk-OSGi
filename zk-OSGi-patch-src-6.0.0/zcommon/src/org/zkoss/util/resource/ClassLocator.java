...
/**
 * The locator searches the current thread's context class loader, and then this
 * class's class loader.
 * 
 * <p>
 * It is important to use this locator if you want to load something in other
 * jar files.
 * 
 * <p>
 * Since this locator is used frequently, {@link Locators#getDefault} is
 * provided to return an instance of this class,
 * 
 * @author tomyeh
 */
public class ClassLocator implements XMLResourcesLocator {
	...
	public Enumeration<URL> getResources(String name) throws IOException {
		...
		Enumeration<URL> en = ClassLoader.getSystemResources(name);
		if (en.hasMoreElements()) {
			return en;
		} else {
			return Activator.getResources(name);
		}		
	}
	...
	public List<Resource> getDependentXMLResources(String name, String elName,
		...
		final List<Resource> rcs = new LinkedList<Resource>(); // a list of
																// Document
	...
	private static void resolveDependency(XMLResource xr, List<Resource> rcs,
			Map<String, XMLResource> rcmap, Set<String> resolving) {
		if (!resolving.add(xr.name))
			throw new IllegalStateException("Recusrive reference among "
					+ resolving);
	...
	private static class XMLResource {
		...
		private XMLResource(URL url, String elName, String elDepends)
			...
			try {
				...
				final IOException ioex = new IOException("Unable to load "
						+ url);
				ioex.initCause(ex);
				throw ioex;
			}
